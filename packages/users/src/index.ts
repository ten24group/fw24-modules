...
import { NEW_USER_GROUPS, TABLE_NAME, ADMIN_USER_GROUPS, UserSchemaDIToken } from './const';
...
export class UsersModule extends AbstractFw24Module {
    ...
    getExportedEnvironmentVariables(): Map<string, string> {
        ...
        envVars.set(ADMIN_USER_GROUPS, Array.isArray(this.config.adminUserGroups) ? this.config.adminUserGroups.join(',') : this.config.adminUserGroups);
        return envVars;
    }
}