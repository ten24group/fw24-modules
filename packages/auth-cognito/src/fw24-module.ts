import { AbstractFw24Module, AuthConstruct, createLogger, FW24Construct, ILogger, LambdaFunctionProps, TriggerType, type ArrayElement } from '@ten24group/fw24';
import { type UserPoolOperation } from 'aws-cdk-lib/aws-cognito';
import { join } from 'path';
import { AuthModulePolicy_AllowCreateUserAuth, CUSTOM_MESSAGE_ADMIN_CREATE_USER, CUSTOM_MESSAGE_AUTHENTICATE, CUSTOM_MESSAGE_FORGOT_PASSWORD, CUSTOM_MESSAGE_RESEND_CODE, CUSTOM_MESSAGE_SIGN_UP, CUSTOM_MESSAGE_UPDATE_USER_ATTRIBUTE, CUSTOM_MESSAGE_VERIFY_USER_ATTRIBUTE, CUSTOM_SUBJECT_ADMIN_CREATE_USER, CUSTOM_SUBJECT_AUTHENTICATE, CUSTOM_SUBJECT_FORGOT_PASSWORD, CUSTOM_SUBJECT_RESEND_CODE, CUSTOM_SUBJECT_SIGN_UP, CUSTOM_SUBJECT_UPDATE_USER_ATTRIBUTE, CUSTOM_SUBJECT_VERIFY_USER_ATTRIBUTE } from './const';
import { defaultUIConfig } from './default-ui-config';
import { IAuthModuleConfig } from './interfaces';
import { AuthUIConstruct } from './ui-construct';
...
export class AuthModule extends AbstractFw24Module {
  ...
  constructor(protected readonly config: IAuthModuleConfig) {
    ...
    // If UI hosting is enabled, register the UI construct after Cognito
    if (config.ui?.enabled) {

      // Merge user UI config with canonical defaults
      const uiConfigWithDefaults = ({
        ...defaultUIConfig,
        ...config.ui!,
        theme: { ...defaultUIConfig.theme, ...(config.ui!.theme || {}) },
        features: { ...defaultUIConfig.features, ...(config.ui!.features || {}) },
        i18n: { ...defaultUIConfig.i18n, ...(config.ui!.i18n || {}) },
      } as NonNullable<IAuthModuleConfig[ 'ui' ]>);

      const ui = new AuthUIConstruct(uiConfigWithDefaults);
      // Ensure UI resources are created after the Cognito construct
      ui.dependencies = [ 'auth-cognito' ];
      this.constructs.set('auth-ui', ui);
    }
  }
